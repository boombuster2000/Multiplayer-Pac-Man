...
{
    static_assert(std::is_base_of<GridTile, T>::value, "T must derive from UIComponents::GridTile");
  ...
  public:
    ...
    Grid(Vector2Ex<size_t> arraySize, Vector2Ex<float> tileDimensions, Vector2Ex<float> anchorPointPosition,
         UIComponents::AnchorPoint anchorPoint, Vector2Ex<float> spacing, Args &&...tileArgs)
